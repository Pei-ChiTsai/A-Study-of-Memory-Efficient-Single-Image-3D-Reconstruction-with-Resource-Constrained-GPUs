...
class TriplaneNeRFRenderer(BaseModule):
    ...
    def interpolate_triplane(self, triplane: torch.Tensor, resolution: int):
        ...
        verts2D = verts2D.expand(3, -1, -1, -1) #(3, resolution, resolution, 2)
        return F.grid_sample(triplane, verts2D, align_corners=False,mode="bilinear") # [3 40 H W] xy xz yz

    def block_based_marchingcube(self, decoder: torch.nn.Module, triplane: torch.Tensor, resolution: int, threshold, block_resolution = 128) -> torch.Tensor: ##128
        ...
        for x in range(0, resolution - 1, block_resolution):
            size_x = resolution - x if x + block_resolution >= resolution else block_resolution + 1 
    ...
    def _forward(
        ...
        **kwargs,
        ...
        rays_o = rays_o.view(-1, 3) # Ray origin (N,3)
        rays_d = rays_d.view(-1, 3) # Ray direction (N,3)
        n_rays = rays_o.shape[0]

        #Calculate the intersection points of each ray with the scene's bounding box, identifying valid rays.
        t_near, t_far, rays_valid = rays_intersect_bbox(rays_o, rays_d, self.cfg.radius)
        # P = O + tD, where P is a point on the ray, O is the ray's origin, D is the ray's direction, and t is a parameter.
        t_near, t_far = t_near[rays_valid], t_far[rays_valid]

        #Uniformly distribute samples_per_ray + 1 points between 0 and 1.
        ...
        #Calculate the world coordinates of the sampling points using the depth values and the ray equation.
        xyz = (
            rays_o[:, None, :] + z_vals[..., None] * rays_d[..., None, :]
        )  # (N_rays, N_sample, 3)

        #Query the color and density of the sampling points.
